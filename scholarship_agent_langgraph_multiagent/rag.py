...
── Graph flow ───────────────────────────────────────────────────────────────────
  ...
  (C) conditional   — routing function decides at runtime
        ...
        │  - pass / fail          │
        └───────┬───────────┬─────┘
   (C) PASS     │           │ FAIL (C)  ← route_after_critic()
                │           │
                v           v
      ┌──────────────┐   ┌──────────────┐
      │     END      │   │   ROUTER     │
      ...
      └──────────────┘   └──────┬───────┘
          ...
          wc<2  │                                │ rc<2
                v                                v
     ┌──────────────────┐  (D)       ┌──────────────────┐
     │ WRITER RETRY     │◄───────────│ RESEARCH RETRY   │
     │ (fix answer)     │            │ (new retrieval)  │
     └─────────┬────────┘            └──────────────────┘
               │ (D)
               v
            CRITIC  ← LangGraph supports cycles; writer_count /
                       research_count guard against infinite loops
